...
interface TelaPedidoFilialProps {
   ...
   * se o item é um produto ou um suprimento" — a Lista de Produção
   * ganha a mesma pergunta que já existia em Reposição). Esta tela não
   * tem carrinho de suprimento nem envio — cadastrar aqui só inclui no
   * catálogo, para já aparecer pronto quando alguém for pedir em
   * Reposição (ver PainelFornadasFilial.tsx).
   */
...
export function TelaPedidoFilial({
  ...
  /**
   * Cadastro relâmpago de SUPRIMENTO a partir desta mesma busca
   * (set/2026). Sem carrinho de suprimento nesta tela — só registra no
   * catálogo e fecha o cartão; a quantidade e o envio acontecem em
   * Reposição (PainelFornadasFilial.tsx), não aqui.
   */
  async function cadastrarSuprimentoDaBusca(
    nome: string,
    segmento: string,
    remover?: () => void
  ) {
          ...
          <button
            ...
            onClick={() =>
              emAndamento.tipo === "produto"
                ...
                : void cadastrarSuprimentoDaBusca(emAndamento.nome, emAndamento.categoria!, emAndamento.remover)
            }
          >
